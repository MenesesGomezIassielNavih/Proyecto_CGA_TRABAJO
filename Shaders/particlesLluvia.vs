...
vec3 randomInitialPosition() {  //para lluvia se cambio posicion inicial
    float offsetX = mix(-50.0, 50.0, texelFetch(RandomTex, 2 * gl_VertexID, 0).r); //para que se distribuya en rectangulo
	float offsetZ = mix(-50.0, 50.0, texelFetch(RandomTex, 2 * gl_VertexID + 1, 0).r); //se agrego para lluvia
    return Emitter + vec3(offsetX, 0, offsetZ); //se agrego el offsetZ
}
